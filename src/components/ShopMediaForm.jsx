...
function ShopMediaForm({ shop }) {
  ...
  const handleUpdateLogo = async (event) => {
    ...
    try {
      const response = await axios.post(
        "https://api-bekkah.onrender.com/api/seller/shop/logo",
        formData,
        {
          headers:{
  ...
  const handleUpdateBanner = async (event) => {
    ...
    try {
      const response = await axios.post(
        "https://api-bekkah.onrender.com/api/seller/shop/banner",
        formData,
        {
  ...
  return (
    <div>
      <h2 className='text-xl font-bold' className="text-2xl font-bold mb-4">
      ...
      <div className="flex items-start justify-between px-4">
        <form className="mb-8 w-full" onSubmit={(e) => handleUpdateBanner(e)}>
          ...
          <div className="mt-8">
            <button
              type="submit"
              className="bg-[#1874BD] hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
            >
              Update Banner
            </button>
          </div>
