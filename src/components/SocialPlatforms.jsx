  ...
  const handleAddSocialLink = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://api-bekkah.onrender.com/api/seller/shop/social",
        newSocialLink,
        {
          headers: {
  ...
  const handleDeletePlatform = async (name) => {
    try {
      const response = await axios.delete(
        `https://api-bekkah.onrender.com/api/seller/shop/social/${name}`,
        {
          headers: {
            "x-auth-token": localStorage.getItem("sellerAuthToken"),
          },
    ...
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleAddSocialLink} className="mb-8">
        <div className="flex mb-4">
          ...
          <button
            type="submit"
            className="bg-[#1874BD] hover:bg-blue-700 text-white font-bold py-2 px-4 rounded ml-4"
          >
